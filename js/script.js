SCRIPT = [
    {
        '0': {
            'TEXT': ["Well, hello, mister detective."],
            'EVENT_TYPE': 'NORMAL',
            'TEXT_SPEED': 'NORMAL'
        }
    },
    {
        '0': {
            'TEXT': ["How have you been?"],
            'EVENT_TYPE': 'NORMAL',
            'TEXT_SPEED': 'NORMAL'
        }
    }, 
    {
        '0': {
            'TEXT': [
                "You've had quite the ordeal lately haven't you?", 
                "It makes for quite an excellent story."
            ], 
            'EVENT_TYPE': 'NORMAL',
            'TEXT_SPEED': 'NORMAL'
        }
    },
    {
        '0': {
            'TEXT': ["Shall we reminisce about it together?"],
            'EVENT_TYPE': 'NORMAL',
            'TEXT_SPEED': 'NORMAL'
        }
    },
    {
        '0': {
            'TEXT': ["The tale of that time where the supernatural workings of the underworld came in contact with the life of the ace detective, whose name was..."],
            'EVENT_TYPE': 'NAME',
            'TEXT_SPEED': 'NORMAL'
        }
    },
    {
        '0': {
            'TEXT': ["...playerName0 playerName1, was it? A nice name like that."],
            'EVENT_TYPE': 'NORMAL',
            'TEXT_SPEED': 'NORMAL'
        }
    },
    {
        '0': {
            'TEXT': ["Well, then, detective, you remember very well how it started, don't you? It was a night of stormy weather, when that case reached your hands..."],
            'EVENT_TYPE': 'NORMAL',
            'TEXT_SPEED': 'NORMAL'
        }
    },
    {
        '0': {
            'TEXT': [
                '"Where in the name of everything holy does that much rain come from?!"', 
                'You were sitting on your table, mindlessly checking the notes of the last case you solved, when you heard your boss, the tall, slightly overweight, gray-haired commissioner Edmund "Mundie" Dunkirk, complain for the Nth time in just the last forty minutes.'
            ],
            'EVENT_TYPE': 'NORMAL',
            'TEXT_SPEED': 'NORMAL' 
        }
    },
    {
        '0': {
            'TEXT': ['It was hard to blame him, as you yourself would be sleeping warmly in bed if it was your choice, however, you had no umbrella, so it was better to avoid drenching your uniform in the rain. That meant withstanding boredom, by reading a report on a case that was already closed successfully for a while now, in your journal.'],
            'EVENT_TYPE': 'NORMAL',
            'TEXT_SPEED': 'NORMAL'
        }
    },
    {
        '0': {
            'TEXT': [
                'The boss approached you, shaking a brownish file folder in his large hand.', 
                'Mundie: - Hey, playerName1! If you want to read at work, at least read a case that is still open!'
            ],
            'EVENT_TYPE': 'NORMAL',
            'TEXT_SPEED': 'NORMAL',
            'CHOICES': {
                '0': '> New job for me?', 
                '1': '> Sorry, sir...'
            }
        }
    },
    {
        '0': {
            '0.0': {
                'TEXT': ['Mundie: Yeah, and this is a nasty-looking one. Hope your stomach\'s ready.'],
                'EVENT_TYPE': 'NORMAL',
                'TEXT_SPEED': 'NORMAL'
            },
            '0.1': {
                'TEXT': ['Mundie: HAHAHAHA! Got scared, did you, chum?! Take a good look at this, but prepare your stomach. This case is gut-wrenching.'],
                'EVENT_TYPE': 'NORMAL',
                'TEXT_SPEED': 'NORMAL'
            }
        },
    },
    {
        '0': {
            'TEXT': ['Mundie threw the folder at your table without much care, and waited with crossed arms to hear your opinion. You quickly opened the file and carefully removed its contents.'],
            'EVENT_TYPE': 'NORMAL',
            'TEXT_SPEED': 'NORMAL'
        }
    },
    {
        '0': {
            'TEXT': [
                'Three black-and-white pictures were organised in your table now. Two men and one woman, though their physical traits were barely recognizable: their bodies were horribly burnt to the point where their raw flesh was clearly visible over the barely existent skin. Their necks were torn inside out, and their chests, arms and legs were covered in long gashes reminiscent of claws and fangs. The blood was now dry, but when those wounds were produced, it certainly flowed like a waterfall into the dirty sidewalks where the bodies laid.',
                'Truly, a disgusting portrait of violence.'
            ],
            'EVENT_TYPE': 'NORMAL',
            'TEXT_SPEED': 'NORMAL'
        }
    },
    {
        '0': {
            'TEXT': ['"Mauled and burned, all of them." you said to yourself, in a low voice. Rambling about the details of every case that would reach your hands was a particular quirk of yours, that earned some mocking from your colleagues. None of it was mean-willed, however, as your deductive reasoning was more than enough to make up for that particular thought process. "Do we know which of the two actually killed each victim?"'],
            'EVENT_TYPE': 'NORMAL',
            'TEXT_SPEED': 'NORMAL'
        }
    },
    {
        '0': {
            'TEXT': [
                "Mundie: The boys from Scotland Yard already collected the bodies. They\'ll be in the Kew Mortuary for autopsy. I\'ve already scheduled a meeting. Be there by 8am, they\'ll let you investigate the bodies yourself. Afterwards, you\'ll join me in the murder locations, so we can more thoroughly analyse what took place. Don't be late.",
                '\"I\'ll be there", you answered. Three deaths of such vile nature were nothing to celebrate, of course, but for now, you at least had something to take your mind off of the goddamned rain. '
            ],
            'EVENT_TYPE': 'NORMAL',
            'TEXT_SPEED': 'NORMAL'
        }
    },
    {
        '0': {
            'TEXT': ['You opened your journal and marked a new section to this case. (You can use "L" to open your Journal Log)'],
            'EVENT_TYPE': 'NORMAL',
            'TEXT_SPEED': 'NORMAL'
        }
    },
    {
        '0': {
            'TEXT': ['DAY 1 - Kew Mortuary, 7:50am the next day.'],
            'EVENT_TYPE': 'NORMAL',
            'TEXT_SPEED': 'SCENARIO_CHANGE'
        }
    },
    {
        '0': {
            'TEXT': [
                'The white walls, the metal fences surrounding the building, culminating in an arc above the entrance to the gardens of the chapel-looking structure, gave off the usual feeling of a morgue: inevitably eerie, unexplainably uncomfortable, and filled with a sepulchral silence that made the air heavier.',
                'It wasn\'t your first, and hardly would be your last time in a morgue, but this was a feeling that would never really disappear.'
            ],
            'EVENT_TYPE': 'NORMAL',
            'TEXT_SPEED': 'NORMAL'
        }
    },
    {
        '0': {
            'TEXT': [
                'Kind receptionist: You are early, detective playerName1. Did you fall off your bed?',
                '\"Something like that." you answered with a smile. You were used to the black-haired young lady with smiley eyes behind the desk, shooting her fingertips at the buttons of the typewriter like a human gatling gun. \"I\'m here for my appointment with Dr Francis. I know I\'m early, but could you get me in ?"',
                'Kind receptionist: With pleasure.'
            ],
            'EVENT_TYPE': 'NORMAL',
            'TEXT_SPEED': 'NORMAL'
        }
    },
    {
        '0': {
            'TEXT': [
                'A few minutes later, you were inside the morgue, where the walls were covered by rectangular silver freezers, keeping nameless corpses inside of them for a variety of reasons. Shaking your hand was the doctor Carl Francis, a white haired old man, not very tall, of skeletal composition, and a pair of cold, dull grey eyes that revealed how many years that man spent among the dead, disassociating himself further and further from his deceased similars.',
                'Dr Francis: Always a pleasure to have you here. Kindly follow me.'
            ],
            'EVENT_TYPE': 'NORMAL',
            'TEXT_SPEED': 'NORMAL'
        }
    },
    {
        '0': {
            'TEXT': [
                'His greeting was generic. His welcome, fully devoid of emotion. With you following close by, the doctor opened three of the containers, one beside the other, and pulled out the metal litters where the nude bodies laid.',
                'Dr Francis: Very well then. Whatever shall I tell you, detective ?'
            ],
            'EVENT_TYPE': 'INFO_GATHERING',
            'TEXT_SPEED': 'NORMAL',
            'CHOICES': {
                '0': '> What kind of injury are we talking about here?', 
                '1': '> What specifically killed them?',
                '2': '> Is there any difference between the wounds in the bodies?',
                '3': '> Tell me about the victims.',
                '4': '> That will be all, thank you.'
            },
        }
    },
    {
        '0': {
            '0.0': {
                'TEXT': [
                    'Dr Francis: Well, let\'s start with the cuts on their flesh. Given the angulation, depth and shape, those are bites and gashes caused by fangs and claws. The cuts are not clean enough to be caused by any kind of blade I know. The problem is, there is no animal in London that fits these wounds.',
                    '\"Is it possible that a big dog attacked them?\" you asked. It made more sense than supposing a big predator invaded the town and went unnoticed until now.'
                ],
                'EVENT_TYPE': 'NORMAL',
                'TEXT_SPEED': 'NORMAL' 
            },
            '0.1': {
                'TEXT': [
                    'Dr Francis: You mean if it was either the gashes or the burns that happened first?',
                    'You nodded. There had to be something, a detail, anything, that would point towards a clue.',
                    'Dr Francis had a slightly disappointed look on his usually impassible face, as if knowing his next answer would confuse you even further.',
                    'Dr Francis: Given the state of their blood and clothes... both happened simultaneously.'
                ],
                'EVENT_TYPE': 'NORMAL',
                'TEXT_SPEED': 'NORMAL'
            },
            '0.2': {
                'TEXT': [
                    'Dr Francis: Except for the specific placements of the wounds, they are the exact same. It\'s safe to assume that the same thing killed all three of the victims.',
                    'Dr Francis: Now, is there anything else you\'d like to ask?',
                ],
                'EVENT_TYPE': 'NORMAL',
                'TEXT_SPEED': 'NORMAL'
            },
            '0.3': {
                'TEXT': [
                    'Dr Francis: Sadly, the police couldn\'t identify the bodies due to their state, so I don\'t even know their names. You might find someone who will recognize them in the crime scenes, though...',
                    'Dr Francis: Now, is there anything else you\'d like to ask?'
                ],
                'EVENT_TYPE': 'NORMAL',
                'TEXT_SPEED': 'NORMAL' 
            }
        }
    },
    {
        '0': {
            '0.0': {
                'TEXT': [
                    'Dr Francis: This, my friend, is the problem. The dentition matches that of a canine, but the teeth marks are too deep to even be caused by a wolf - let alone a dog. Besides, there are other injuries that seem to be caused by claws, and canines don\'t usually attack with their claws, they hardly grow much. And then... we have the lady here.',
                    'The doctor motioned towards the only female between the three victims, pointing out the enormous gashes in her left shoulder and in the right side of her neck.'
                ],
                'EVENT_TYPE': 'NORMAL',
                'TEXT_SPEED': 'NORMAL' 
            },
            '0.1': {
                'TEXT': [
                    '\"Simultaneously? So we have a predator of blazing skin and big fangs and claws just stalking and murdering people during the night?\". Unbelievable. You weren\'t sure if there was even a way to say this sentence out loud without it getting ridiculous.',
                    'The doctor\'s analysis was getting weirder by the second, and yet, there were the bodies, right in front of you, backing up everything he had to say.'
                ],
                'EVENT_TYPE': 'NORMAL',
                'TEXT_SPEED': 'NORMAL'
            }
        }
    },
    {
        '0': {
            '0.0': {
                'TEXT': [
                    'Dr Francis: These bite marks, given their state when compared to other bruises, seem to be made at roughly the same time.',
                    '\"Then it\'s possible she was attacked by two of our mysterious creatures ?\"',
                    'Dr Francis: Either that, or whatever attacked her had two heads. Now, about the burn marks... they\'re third degree burns or even worse. A burning this intense would need the victims bodies to be drenched in gasoline, or some other inflammable substance, which wasn\'t the case.',
                ],
                'EVENT_TYPE': 'NORMAL',
                'TEXT_SPEED': 'NORMAL'
            },
            '0.1': {
                'TEXT': [
                    'Dr Francis: I\'m sure there is a perfectly rational explanation for this. What I\'m telling you is just what we found in the autopsy, detective.',
                    'Dr Francis: Now, is there anything else you\'d like to ask?'
                ],
                'EVENT_TYPE': 'NORMAL',
                'TEXT_SPEED': 'NORMAL'
            }
        }
    },
    {
        '0': {
            '0.0': {
                'TEXT': ['Dr Francis: Now, is there anything else you\'d like to ask?'],
                'EVENT_TYPE': 'NORMAL',
                'TEXT_SPEED': 'NORMAL'
            }
        }
    },
    {
        '0': {
            'TEXT': [
                '\"That will be all. Thank you.\" you said. In all honesty, all the information you got made things even more confusing than how they were with the lack of context. Your mind was nurturing some theories about a culprit using trained hounds to murder victims and burning their bodies to hide evidence, or perhaps the burning was the murder method, and the dogs were supposed to eat the bodies afterwards, but didn\'t have enough time. However, with the doctor\'s input, both were now debunked.',
            ],
            'EVENT_TYPE': 'NORMAL',
            'TEXT_SPEED': 'NORMAL'    
        }
    },
    {
        '0': {
            'TEXT': [
                'Dr Francis: Happy to be of service. Will you be investigating the crime scene afterwards?',
                '\"Yes\" you confirmed. \"We\'ll be sending notice to the mortuary as soon as we identify the victims. Farewell, doctor.\"',
                'Dr Francis:  Good day, detective. '
            ],
            'EVENT_TYPE': 'NORMAL',
            'TEXT_SPEED': 'NORMAL'
        }
    },
    {
        '0': {
            'TEXT': ['The first crime scene, Westminster, 8:30am'],
            'EVENT_TYPE': 'NORMAL',
            'TEXT_SPEED': 'SCENARIO_CHANGE'
        }
    },
    {
        '0': {
            'TEXT': [
                'Mundie: Damn that old fart! What? He thinks that Kerberus killed them?',
                '\"Well, he joked about one of the creatures having two heads, but he was inconclusive about what the attacker could be.\" you explained. Having arrived at the scene about two minutes ago, you were catching up with Mundie on what you discovered from Dr Francis.',
            ],
            'EVENT_TYPE': 'NORMAL',
            'TEXT_SPEED': 'NORMAL'
        }
    },
    {
        '0': {
            'TEXT': [
                'Mundie wiped his forehead with his sleeve, and pulled his black cap\'s edge lower above his eyes, to cover the sun.',
                'Mundie: God damn it all to hell. Well, anyways, here\'s the profile of the victim. I doubt you\'ll find any correlation between the victim and the fucking fire werewolf or whatzitcalled that killed him, but maybe you can find something that points to a culprit we can actually believe exists. Feel free to write down relevant notes in it.',
            ],
            'EVENT_TYPE': 'ITEM_ADDED',
            'ITEM_ADDED_TEXT': 'Jacob Jones\' profile was added to the inventory.',
            'ITEM_ADDED_ID': 0,
            'TEXT_SPEED': 'NORMAL'
        }
    },
]